...
class BackendNotifierTest extends TestCase {
	...
	private function sortParticipantUsers(array $message): array {
		...
		if ($message['type'] === 'message' && $message['message']['data']['type'] === 'switchto') {
			foreach ($message['message']['data']['switchto'] as $room => $sessionIds) {
				usort($sessionIds, static function ($a, $b) {
					return $a <=> $b;
				});
				$message['message']['data']['switchto'][$room] = $sessionIds;
			}
		}
		return $message;
	}
